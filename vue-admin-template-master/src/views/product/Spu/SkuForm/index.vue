  ...
  <div>
    <el-form label-width="100px">
      ...
      <el-form-item label="平台属性">
        <el-form :inline="true">
          <el-form-item :label="attr.attrName" label-width="100px" v-for="attr in attrInfoList" :key="attr.id">
      ...
      <el-form-item label="销售属性">
        <el-form :inline="true">
          <el-form-item :label="saleAttr.saleAttrName" label-width="100px" v-for="saleAttr in spuSaleAttrList"
            :key="saleAttr.id">
      ...
      <el-form-item label="图片列表">
        <el-table border style="width:100%" :data="spuImageList" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="图片" prop="" width="width">
  ...
  methods: {
    async getData(category1Id, category2Id, spu) {
      ...
      if (result.code == 200) {
        ...
        list.forEach(item => {
          item.isDefault = 0; //0代表的是显示设置默认按钮
        });
    ...
    handleSelectionChange(params) {
      // 获取到用户选择的图片的信息数据，但是需要注意，当前收集的数据当中，缺少isDefault字段
      // console.log(params);
